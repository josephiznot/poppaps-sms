...
tournamentAdmin.get('/',async c=>{
  ...
  const cards = await Promise.all(plans.slice(0,8).map(async p=>{
    ...
    const roster = offers.length ? `<table><caption>Seat offers · eight seats maximum</caption><thead><tr><th>Player</th><th>Seat</th><th>Reply by</th></tr></thead><tbody>`+offers.map(o=>`<tr><td>${esc(o.display_name ?? 'Player')}</td><td>${esc(statusLabel(o.state))}</td><td>${when(o.response_deadline)}</td></tr>`).join('')+`</tbody></table>`:'';
    const deliveryRows = deliveries.map(d=>`<tr><td>${esc(board.find(b=>b.member_phone===d.recipient)?.display_name ?? 'Player')}</td><td>${esc(statusLabel(d.state))}${d.last_error ? `<br><span class="muted">${esc(d.last_error)}</span>` : ''}</td><td>${d.state==='FAILED' && d.retryable ? `<form method="post" action="/admin/tournament/retry"><input type="hidden" name="delivery_id" value="${esc(d.id)}"><button>Retry failed text</button></form>` : (d.state==='UNKNOWN' || d.state==='ACCEPTED') && d.provider_sid ? `<form method="post" action="/admin/tournament/reconcile"><input type="hidden" name="delivery_id" value="${esc(d.id)}"><button>Check provider status</button></form>` : d.state==='UNKNOWN' ? 'Check the provider log before contacting this player; receipt unavailable.' : ''}</td></tr>`).join('');
