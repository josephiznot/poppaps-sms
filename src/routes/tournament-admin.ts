...
tournamentAdmin.get('/',async c=>{
  ...
  const cards = await Promise.all(plans.slice(0,8).map(async p=>{
    ...
    const roster = offers.length ? `<table><caption>Seat offers · eight ranked seats plus the host</caption><thead><tr><th>Player</th><th>Seat</th><th>Reply by</th></tr></thead><tbody>`+offers.map(o=>`<tr><td>${esc(o.display_name ?? 'Player')}${o.is_host_offer ? ' · Host' : ''}</td><td>${esc(statusLabel(o.state))}</td><td>${o.is_host_offer ? 'Guaranteed until play; FOLD may decline' : when(o.response_deadline)}</td></tr>`).join('')+`</tbody></table>`:'';
