...
function fixture(){
  ...
  sqlite.exec(`INSERT INTO games(id,starts_at,location,created_at,results_recorded_at) VALUES('qualifying','2026-09-21T23:30:00.000Z','Lounge','2026-09-01','2026-09-22');`);
  for(let i=0;i<10;i++){
    ...
    sqlite.prepare('INSERT INTO points_ledger(id,member_phone,game_id,points,place,awarded_at) VALUES(?,?,?,?,?,?)').run(`p${i}`,phone,'qualifying',10-i,1,'2026-09-21T23:30:00.000Z');
  }
...
describe('automatic tournament integrated workflow',()=>{
  it('schedules October 12, closes once, isolates uncertain sends and replaces a decline',async()=>{
    ...
    expect(initial.planned_starts_at).toBe('2026-10-12T23:30:00.000Z');
    expect(initial.qualification_cutoff).toBe('2026-09-28T15:00:00.000Z');
    await tickTournaments(env,new Date('2026-09-28T15:01:00.000Z'));
    await tickTournaments(env,new Date('2026-09-28T15:02:00.000Z'));
    ...
    let calls=0;
    await drainOutbox(env,new Date('2026-09-28T15:02:00.000Z'),{paceMs:0,clock:()=>new Date('2026-09-28T16:02:00.000Z'),transport:async()=>{calls++;if(calls===3)throw new Error('Connection lost after submit');return {sid:`SM${calls}`,status:'queued'};}});
    expect((await listDeliveries(db,initial.id)).filter(d=>d.state==='UNKNOWN').length).toBe(1);
    await drainOutbox(env,new Date('2026-09-28T16:02:00.000Z'),{paceMs:0,clock:()=>new Date('2026-09-28T16:02:00.000Z'),transport:async()=>{throw new Error('Unexpected repeat send');}});
    ...
    await respondToTournamentOffer(env,first.member_phone,'DECLINE',new Date('2026-09-29T15:00:00.000Z'));
    await tickTournaments(env,new Date('2026-09-29T16:00:00.000Z'));
    ...
    expect((await respondToTournamentOffer(env,first.member_phone,'CONFIRM',new Date('2026-09-29T17:00:00.000Z'))).outcome).not.toBe('CONFIRMED');
  });
